...
class _MarkdownTableBlock extends StatelessWidget {
  ...
  Future<bool> _savePngBytesToGallery(Uint8List bytes) async {
    final result = await ImageGallerySaverPlus.saveImage(
      bytes,
      quality: 100,
      name: 'nastechai-table-${DateTime.now().millisecondsSinceEpoch}',
    );
    if (result is Map) {
  ...
  Future<File> _writeTableImageTempFile(Uint8List bytes) async {
    ...
    final file = File(
      p.join(
        dir.path,
        'nastechai-table-${DateTime.now().millisecondsSinceEpoch}.png',
      ),
    );
  ...
  Future<bool> _writePngToClipboard(Uint8List bytes) async {
    try {
      ...
      if (clipboard != null) {
        final item = DataWriterItem(suggestedName: 'nastechai-table.png');
  ...
  Future<bool> _saveCachedMermaidPng(Uint8List bytes) async {
    try {
      ...
      final result = await ImageGallerySaverPlus.saveImage(
        bytes,
        quality: 100,
        name: 'nastechai-mermaid-${DateTime.now().millisecondsSinceEpoch}',
      );
      if (result is Map) {
        final isSuccess =
