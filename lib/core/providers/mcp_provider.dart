...
import '../services/mcp/nastechai_fetch/nastechai_fetch_server.dart';
...
class McpProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    // Ensure built-in @nastechai/fetch is present by default
  ...
  void _ensureBuiltinFetchServerPresent() {
    final exists = _servers.any(
      (s) =>
          ...
          s.name == '@nastechai/fetch' ||
          s.id == 'nastechai_fetch',
    );
    if (exists) return;
    final cfg = McpServerConfig(
      id: 'nastechai_fetch',
      enabled: true,
      name: '@nastechai/fetch',
      transport: McpTransportType.inmemory,
      ...
      if (serversFromMap != null) {
        ...
        serversFromMap.forEach((id, cfgAny) {
          ...
          if (typeLower == 'inmemory') {
            // Built-in @nastechai/fetch control via isActive; ignore name mismatches silently
            builtinSeen = true;
