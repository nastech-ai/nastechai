...
class _ImageViewerPageState extends State<ImageViewerPage>
  ...
  Future<void> _saveCurrent() async {
    ...
    try {
      ...
      final name = 'nastechai-${DateTime.now().millisecondsSinceEpoch}';
      ...
      if (src.startsWith('data:')) {
        ...
        if (i != -1) {
          ...
          temp = await File(
            p.join(
              tmp.path,
              'nastechai_${DateTime.now().millisecondsSinceEpoch}.png',
            ),
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300) {
          ...
          temp = await File(
            p.join(
              tmp.path,
              'nastechai_${DateTime.now().millisecondsSinceEpoch}${ext.isNotEmpty ? ext : '.jpg'}',
            ),
          ).create(recursive: true);
    ...
    if (!ok) {
      try {
        ...
        if (path == null) {
          ...
          path = p.join(
            dir.path,
            'nastechai_clip_${DateTime.now().millisecondsSinceEpoch}$ext',
          );
  ...
  Future<void> _saveCurrentDesktop() async {
    ...
    try {
      ...
      final defaultName = 'nastechai-${DateTime.now().millisecondsSinceEpoch}$ext';
