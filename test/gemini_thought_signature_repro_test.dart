        ...
        if (requestCount == 1) {
          ...
          request.response.write(
            'data: ${jsonEncode(_streamChunk([
              _functionCallPart(name: 'google_search', args: {'query': 'NasTech AI fetch'}),
            ]))}\n\n',
          );
          request.response.write(
          ...
          messages: const [
            {
              'role': 'user',
              'content':
                  'Search the web for the NasTech AI fetch server docs, then summarize them.',
            },
          ],
          tools: const [
            {'google_search': {}},
      ...
      () async {
        ...
        server.listen((request) async {
          ...
          if (requestCount == 1) {
            ...
            request.response.write(
              'data: ${jsonEncode(_streamChunk([
                _functionCallPart(name: 'google_search', args: {'query': 'NasTech AI fetch'}, thoughtSignature: 'sig-google-search'),
              ]))}\n\n',
            );
            request.response.write('data: [DONE]');
            await request.response.close();
    ...
    test('preserves parallel function calls without inventing signatures', () async {
      ...
      server.listen((request) async {
        ...
        if (requestCount == 1) {
          ...
          request.response.write(
            'data: ${jsonEncode(_streamChunk([
              _functionCallPart(name: 'search_docs', args: {'query': 'NasTech AI fetch'}, thoughtSignature: 'sig-search-docs'),
    ...
    test(
      ...
      () async {
        ...
        final chunks = await ChatApiService.sendMessageStream(
          ...
          modelId: 'gemini-3.1-pro-preview',
          messages: const [
            {'role': 'user', 'content': '查 NasTech AI'},
            {
              'role': 'assistant',
              'content': '\n\n',
              'tool_calls': [
