...
import 'package:nastech_ai/core/services/search/providers/querit_search_service.dart';
import 'package:nastech_ai/core/services/search/search_service.dart';
...
void main() {
  group('Querit search service', () {
    ...
    test('posts filters and parses Querit results', () async {
        ...
        client: MockClient((request) async {
          ...
          return http.Response(
            jsonEncode({
              ...
              'error_msg': '',
              'search_id': 1,
              'query_context': {'query': 'nastechai'},
              'results': {
                'result': [
                  {
                    'title': 'NasTech AI',
                    'url': 'https://example.com/nastechai',
      ...
      expect(result.items.single.title, 'NasTech AI');
      expect(result.items.single.url, 'https://example.com/nastechai');
      expect(
    ...
    test('omits filters when optional values are empty', () async {
      ...
      expect(jsonDecode(captured!.body), {'query': 'nastechai', 'count': 5});
    ...
    test('parses sentence field when snippets are absent', () async {
      ...
      expect(result.items.single.title, 'https://example.com/nastechai');
