...
void main() {
  group('DataSync backup file', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp('nastechai_data_sync_test_');
    ...
    test(
      ...
      () async {
        ...
        final staleWorkDir = Directory('${tmpDir.path}/nastechai_backup_stale');
        await staleWorkDir.create(recursive: true);
        await File('${staleWorkDir.path}/orphan.zip').writeAsString('old');
        await File('${tmpDir.path}/nastechai_backup_old.zip').writeAsString('old');
        ...
        expect(
          await File('${tmpDir.path}/nastechai_backup_old.zip').exists(),
          isFalse,
        );
